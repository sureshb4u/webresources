...
function SylvanCalendar() {
    ...
    this.createEventOnDrop = function (t, date, allDay, ev, ui, resource, elm) {
        ...
        if (wjQuery(elm).attr("type") == 'student') {
          ...
          var allowEventOverlap = self.validateOverlapingEvent(startHour, resource.id, prevStudObj, false);
          ...
          var allowEventOverlap = self.validateOverlapingEvent(startHour, resource.id, teacher[0], false);
          if(allowEventOverlap){
            ...
            if (index != -1) {
              ...
              var allowEventOverlap = self.validateOverlapingEvent(startHour, resource.id, prevStudObj, true);
              if(allowEventOverlap){
            ...
            var techerPrograms = this.getProgramObj(teacherId);
            var prevEvent = this.calendar.fullCalendar('clientEvents', prevEventId);
            var allowEventOverlap = self.validateOverlapingEvent(startHour, resource.id, prevEvent, true);
            if(allowEventOverlap){
