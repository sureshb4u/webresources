...
function SylvanCalendar() {
    ...
    this.populateTAPane = function (teacherData) {
        ...
        for (var i = 0; i < teacherData.length; i++) {
            ...
            var teacherStart = new Date(moment(currentCalendarDate).format('MM-DD-YYYY') + ' ' + teacherStartHour + ":00");
    ...
    this.studentSofConflictCheck = function (t, date, allDay, ev, ui, resource, elm) {
        ...
        if (student[0] == undefined) {
            ...
            for (var sw = 0; sw < this.sofList['Group Facilitation'].length; sw++) {
                if (this.sofList['Group Facilitation'][sw].id == uniqueVal[0] &&
                    this.sofList['Group Facilitation'][sw].start.getTime() == new Date(uniqueVal[1]).getTime()) {
            ...
            if (student[0] == undefined) {
                ...
                for (var sw = 0; sw < this.sofList['Group Instruction'].length; sw++) {
                    if (this.sofList['Group Instruction'][sw].id == uniqueVal[0] &&
                        this.sofList['Group Instruction'][sw].start.getTime() == new Date(uniqueVal[1]).getTime()) {
        ...
        if (resource.id + date != prevEventId) {
            if (t.convertedStudentObj[index]) {
                ...
                if (typeof responseObj == 'boolean' && responseObj) {
                    ...
                    elm.remove();
                    self.updatePrevStudentEvent(prevEvent, stuId, prevEventId);
                    var index = -1;
    ...
    this.updatePrevStudentEvent = function(prevEvent, stuId, prevEventId){
        var self = this;
        if (prevEvent != undefined && prevEvent.length) {
                ...
                if (eventTitleHTML.prop('outerHTML') != undefined) {
                    ...
                    if (checkForNearbyPrevEventBefore.length) {
                        self.removeAllConflictsFromPrevEvent(checkForNearbyPrevEventBefore[0]);
                    }
                    if (checkForNearbyPrevEventAfter.length) {
                        self.removeAllConflictsFromPrevEvent(checkForNearbyPrevEventAfter[0]);
                    }
    ...
    this.generateEventObject = function (args, label) {
        ...
        if (label == "masterTeacherSchedule") {
            ...
            wjQuery.each(args, function (ke, val) {
                ...
                // if (obj.sessionStatus == INVALID_STATUS || obj.sessionStatus == OMIT_STATUS ) {
                //     var index = -1;
                //     for (var i = 0; i < eventObjList.length; i++) {
                //         if (eventObjList[i].id == obj.id &&
                //             eventObjList[i].resourceId == obj.resourceId &&
                        ...
                        wjQuery.each(event, function (k, v) {
                            if (event[k].hasOwnProperty("teachers") && event[k]['teachers'].length != 0) {
                                ...
                                if (index == -1) {
                                    ...
                                    if (event[k].hasOwnProperty("students")) {
                                        ...
                                        wjQuery.each(event[k].students, function (ke, val) {
                                            if(val['sessionStatus'] == EXCUSED_STATUS || val['sessionStatus'] == UNEXCUSED_STATUS){
                                               strikeThroughClass = " excuse-student"; 
                                            }
                                            if (resourceObj.deliveryTypeCode == groupInstruction) {
                                                event[k].title += "<span class='drag-student" + strikeThroughClass + "' eventid='" + eventId + "' uniqueId='" + val.id + "_" + value['resourceId'] + "_" + value['startHour'] + "' id='" + val.id + value['resourceId'] + "' type='studentSession' value='" + val.id + "'>" + val.name + ", " + val.grade + "<i class='material-icons' title='" + val['serviceValue'] + "' style='color:" + val['subjectColorCode'] + "'>location_on</i></span>";
                                            } else {
                                                if (val['pinId'] != undefined) {
                                                    event[k].title += "<span class='draggable drag-student" + strikeThroughClass + "' pinnedId='" + val['pinId'] + "' eventid='" + eventId + "' studUniqueId='" + value['studUniqueId'] + "' uniqueId='" + val.id + "_" + value['resourceId'] + "_" + value['startHour'] + "' id='" + val.id + value['resourceId'] + "' type='studentSession' value='" + val.id + "'><img src='/webresources/hub_/calendar/images/pin.png'/>" + val.name + ", " + val.grade + "<i class='material-icons' style='color:" + val['subjectColorCode'] + "'>location_on</i></span>";
                                                } else {
                                                    event[k].title += "<span class='draggable drag-student" + strikeThroughClass + "' eventid='" + eventId + "' studUniqueId='" + value['studUniqueId'] + "' uniqueId='" + val.id + "_" + value['resourceId'] + "_" + value['startHour'] + "' id='" + val.id + value['resourceId'] + "' type='studentSession' value='" + val.id + "'>" + val.name + ", " + val.grade + "<i class='material-icons' title='" + val['serviceValue'] + "' style='color:" + val['subjectColorCode'] + "'>location_on</i></span>";
                                                }
                                            }
                                        });
                                    }
                                ...
                                wjQuery.each(studentList, function (ke, val) {
                                    ...
                                    if (resourceObj.deliveryTypeCode == groupInstruction) {
                                        event[k].title += "<span class='drag-student" + strikeThroughClass + "' eventid='" + eventId + "' studUniqueId='" + value['studUniqueId'] + "' uniqueId='" + val.id + "_" + value['resourceId'] + "_" + value['startHour'] + "' id='" + val.id + value['resourceId'] + "' type='studentSession' value='" + val.id + "'>" + val.name + ", " + val.grade + "<i class='material-icons' title='" + val['serviceValue'] + "' style='color:" + val['subjectColorCode'] + "'>location_on</i></span>";
                                    } else {
                                        if (val['pinId']) {
    ...
    this.populateStudentEvent = function (studentList, isFromFilter, checkFor1to1) {
        ...
        if (studentList.length) {
            wjQuery.each(studentList, function (key, value) {
                if(self.checkStudentToDisplayOrNot(value['sessionStatus']) || value['isFromMasterSchedule']){
                    var id = value['id'];
                    var name = value['name'];
                    ...
                    var resourceObj = self.getResourceObj(value['resourceId']);
                    var strikeThroughClass = "";
                    if(resourceObj.deliveryTypeCode == personalInstruction && value['sessionStatus'] == EXCUSED_STATUS || value['sessionStatus'] == UNEXCUSED_STATUS){
                       strikeThroughClass = " excuse-student"; 
                    }
                    if (event.length) {
                        wjQuery.each(event, function (k, v) {
                            if (event[k].hasOwnProperty("students") && event[k]['students'].length != 0) {
                                if (checkFor1to1 && ((is1to1 || event[k].students[0].is1to1) && event[k].students[0].id != id)) {
                                    ...
                                    index = event[k].students.map(function (x) {
                                        return x.studUniqueId;
                                    }).indexOf(value['studUniqueId']);
                                    if (index == -1) {
                                        if (resourceObj.deliveryTypeCode == groupInstruction) {
                                            event[k].title += "<span class='drag-student" + strikeThroughClass + "' eventid='" + eventId + "' studUniqueId='" + value['studUniqueId'] + "' uniqueId='" + uniqueId + "' id='" + id + value['resourceId'] + "' type='studentSession' value='" + id + "'>" + name + ", " + grade + "<i class='material-icons' title='" + value['serviceValue'] + "' style='color:" + value['subjectColorCode'] + "'>location_on</i></span>";
                                        } else {
                                            if (value['pinId'] != undefined) {
                                                ...
                                                event[k].title += "<span class='draggable drag-student" + strikeThroughClass + "' eventid='" + eventId + "' studUniqueId='" + value['studUniqueId'] + "' uniqueId='" + uniqueId + "' id='" + id + value['resourceId'] + "' type='studentSession' value='" + id + "'>" + name + ", " + grade + "<i class='material-icons' title='" + value['serviceValue'] + "' style='color:" + value['subjectColorCode'] + "'>location_on</i></span>";
                                            }
                                        }
                            ...
                            if(value['sessionStatus'] != EXCUSED_STATUS && value['sessionStatus'] != UNEXCUSED_STATUS){
                        ...
                        self.addContext("", 'teacherPlaceholder', true, true);
                        if (resourceObj.deliveryTypeCode == groupInstruction) {
                            obj.title += "<span class='drag-student" + strikeThroughClass + "' eventid='" + eventId + "' studUniqueId='" + value['studUniqueId'] + "' uniqueId='" + uniqueId + "' id='" + id + value['resourceId'] + "' type='studentSession' value='" + id + "'>" + name + ", " + grade + "<i class='material-icons' title='" + value['serviceValue'] + "' style='color:" + value['subjectColorCode'] + "'>location_on</i></span>";
                        } else {
                            if (value['pinId'] != undefined) {
                                obj.title += "<span class='draggable drag-student" + strikeThroughClass + "' eventid='" + eventId + "' studUniqueId='" + value['studUniqueId'] + "' pinnedId='" + value['pinId'] + "' uniqueId='" + uniqueId + "' id='" + id + value['resourceId'] + "' type='studentSession' value='" + id + "'><img src='/webresources/hub_/calendar/images/pin.png'/>" + name + ", " + grade + "<i class='material-icons' title='" + value['serviceValue'] + "' style='color:" + value['subjectColorCode'] + "'>location_on</i></span>";
                            } else {
                                obj.title += "<span class='draggable drag-student" + strikeThroughClass + "' eventid='" + eventId + "' studUniqueId='" + value['studUniqueId'] + "' uniqueId='" + uniqueId + "' id='" + id + value['resourceId'] + "' type='studentSession' value='" + id + "'>" + name + ", " + grade + "<i class='material-icons' title='" + value['serviceValue'] + "' style='color:" + value['subjectColorCode'] + "'>location_on</i></span>";
                            }
                        }
                        ...
                        }else if(resourceObj["capacity"] == 1 && (value['sessionStatus'] == EXCUSED_STATUS || value['sessionStatus'] == UNEXCUSED_STATUS)){
                            obj.title += '<span class="student-placeholder-' + obj.deliveryType + '">Student name</span>';
                            self.addContext("", 'studentPlaceholder', true, value['deliveryTypeCode']);
                        }
                        if (value['pinId'] != undefined) {
    ...
    this.omitStudentFromSession = function (element) {
        ...
        if (objStudent[0] != undefined) {
            ...
            if ((typeof (responseObj) == 'boolean' && responseObj) || typeof (responseObj) == 'object') {
                ...
                var prevEvent = this.calendar.fullCalendar('clientEvents', prevEventId);
                self.updatePrevStudentEvent(prevEvent, uniqueIds[0], prevEventId);
            }
        }
        this.openSofPane();
        this.showConflictMsg();
    ...
    this.excuseStudentFromSession = function (element) {
        ...
        if (objStudent[0] != undefined) {
            ...
            if ((typeof (responseObj) == 'boolean' && responseObj) || typeof (responseObj) == 'object') {
                ...
                if (index != -1) {
                    wjQuery(element).remove();
                    this.convertedStudentObj[index]['sessionStatus'] = EXCUSED_STATUS;
                    var value = this.convertedStudentObj[index];
                    self.updateExcuseStudent(element, prevEvent);
                    self.addContext(wjQuery(element).attr("uniqueId"), 'student', true, value['deliveryTypeCode'], value['sessionStatus'], value['sessiontype'], value['isAttended']);
                    self.draggable('draggable');
                }
            }
        }
    ...
    this.addContext = function (uniqueId, labelFor, isPinned, deliveryType, sessionStatus, sessionType, isAttended) {
        ...
        if (labelFor == 'student') {
            if (deliveryType == personalInstruction) {
                ...
                if(sessionStatus != EXCUSED_STATUS){
                    obj.excuse = {
                        name: "Excuse",
                        disabled: self.checkAccountClosure(),
    ...
    // This method will return true/false based on student session status.
    // true means display student
    // false means dont display student 
    this.checkStudentToDisplayOrNot = function(studentSession){
        var self = this;
        var displayStudent = false;
        if(studentSession != undefined){
            if( 
                studentSession == SCHEDULE_STATUS ||
                studentSession == RESCHEDULE_STATUS ||
                studentSession == EXCUSED_STATUS ||
                studentSession == UNEXCUSED_STATUS
            ){
                displayStudent = true;
            }
        }
